...
parser.add_argument("--number_particles", type=int, default=64, help="number of rods to simulate")
parser.add_argument("--animate", action="store_true", help="if flag is called, program will animate system")
...
parser.add_argument("--save", action="store_true", help="save animation and any figures that are created")
parser.add_argument("--fps", type=int, default=7, help="fps of saved animation")
parser.add_argument("--spf", type=int, default=100, help="specify the number of time steps between captured frames")
parser.add_argument("--frames", type=int, default=20, help="specify the total number of captured frames")
parser.add_argument("--spatialCorr", action="store_true", help="calculate the spatial distribution funciton at the end of the simulation")
parser.add_argument("--color_orientation", action="store_true", help="if used, will using orientation as color in animation instead of instantaneous torque")
parser.add_argument("--dirname", default="./", help="save location of animation")
parser.add_argument("--initMethod", default="random", help="initialization method ['random','square', 'hexaxtic', 'two_particles']")
parser.add_argument("--verbose", action="store_true", help="show messages, warnings, etc")
parser.add_argument("--lightTheme", action="store_true", help="uses a light theme instead of a dark theme")
parser.add_argument("--CoM", action="store_true", help="plots only the center of mass of each rod")
...
if args.verbose:
    ...
    print("  ------------------")
    print(f"   > actual packing fraction: {metrics.packingFraction(BoxL, nparticles, sigma):.3f}\n")
...
for t in range(spf * frames):
    ...
    if np.mod(t, spf) == 0:
        ...
        camera.snap()
        if args.verbose:
            print("  >>>  creating frame ", nframes, " of ", args.frames, " frames", end="\r")
        nframes += 1
...
if args.spatialCorr:
    ...
    gravg, radii, _ = metrics.TwoPointCorr(
        [p.cmx for p in ptcls], [p.cmy for p in ptcls], BoxL, max([BoxL/2.8, 5*sigma]), sigma
    )
